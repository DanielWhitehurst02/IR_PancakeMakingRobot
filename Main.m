...
PlaceObject('Plate.ply', [0.8, -0.33, 0.565]);
    ...
    while app.isRunning
       i = i+1;
               ...
               %% we attach the pancake to the gripper
                ...
                endTr13 = transl(-0.5,-0.2,0.8)*troty(pi/2)*trotz(pi/2);
                ...
                mesh_hPancake = PlaceObject('pancake2.ply');
                verticesPancake = get(mesh_hPancake, 'Vertices')
                transformedVertices = [verticesPancake, ones(size(verticesPancake, 1), 1)] * transl([0.8, -0.33, 0.57])';
                set(mesh_hPancake, 'Vertices', transformedVertices(:, 1:3)); % Update pancake's initial position
                ...
                deltaT = 0.05;
                motionHandler2.runRMRC(endTr,time,deltaT);
                delete(mesh_hBlue); %% We delete the previous one
                mesh_hBlue = PlaceObject('BlueSyrupBottle2.ply');
                verticesBlue = get(mesh_hBlue, 'Vertices')
                transformedVertices = [verticesBlue, ones(size(verticesBlue, 1), 1)] * transl([0.6,1.25,0.82])';
                set(mesh_hBlue, 'Vertices', transformedVertices(:, 1:3)); 
                motionHandler2.OpenOrCloseGrippers('close',50);
